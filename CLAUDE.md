...
| T3 | **矢印の接続・方向統一・ノード干渉禁止:** 矢印の始点/終点がブロック・加算点・分岐点に接続し、空白から発生/消滅していないか。同じ役割の矢印群の矢頭方向が統一されているか。**矢印が他のノードを貫通・辺に重なっていないか**。**ルーティング原則: (1) 分岐や複数ノードへの配信は、点と点を斜め直線で結ばない。フォーク形状（水平バー + 垂直ドロップ）で配線する。(2) 直角に折れ曲がった直後に矢頭（鏃）を置かない — 折れ曲がりと矢頭の間に最低 5mm の直線部分（幹）が必要。(3) 図全体の全矢印をチェックする（修正箇所だけでなく既存の矢印も含む）。** 座標計算では `positioning` ライブラリの `below=Xmm` が端-端間ギャップ（中心間距離ではない）であることに注意 | espnow\_dataflow 斜め直線がapi1を貫通、fork直後に矢頭で幹なし |
...
4. **ブロック図の信号フロー:** フィードバック分岐は出力信号線上に junction dot を置く。**分岐・配信のルーティングはフォーク形状（水平バー + 垂直ドロップ）を使う。** 1対多の配信は、幹線から水平バーを伸ばし、各端点から垂直にドロップして接続先の `.north` に到達する。中間にノードがある場合は、水平バーをノード群の上または下に配置し、垂直線をノード間のギャップを通して配線する。点と点を斜め直線で結ぶ直接接続は禁止（途中のノードを貫通するリスクが高い）
