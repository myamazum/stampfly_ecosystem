...
| T3 | **矢印の接続と方向統一:** 矢印の始点がブロック・加算点・分岐点のいずれかに接続し、空白から発生していないか。矢印の終点がブロック・加算点に接続し、空白に消えていないか。図の入出力信号線には必ずラベル（`r(t)`, `y(t)` 等）があるか。**同じ役割の矢印群（例: 入力→ブロック）の矢頭方向が統一されているか**（`{Stealth}-` と `-{Stealth}` の混在に注意） | espnow\_dataflow 上段/下段で矢頭方向が不統一 |
