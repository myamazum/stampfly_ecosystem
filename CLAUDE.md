...
| T3 | **矢印の接続・方向統一・ノード干渉禁止:** 矢印の始点/終点がブロック・加算点・分岐点に接続し、空白から発生/消滅していないか。同じ役割の矢印群の矢頭方向が統一されているか。**矢印が他のノードを貫通・辺に重なっていないか**。**ルーティング原則: 分岐や複数ノードへの配信は、点と点を斜め直線で結ばない。フォーク形状（水平バー + 垂直ドロップ）または直角折れ曲がりで、ブロック間の隙間を通して配線する。** 斜め直線は途中ノードを貫通するリスクが高く、座標計算でも見落としやすい。座標計算では `positioning` ライブラリの `below=Xmm` が端-端間ギャップ（中心間距離ではない）であることに注意 | espnow\_dataflow 斜め直線がapi1を貫通、枠線と重なり |
