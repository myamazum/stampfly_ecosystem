...
| Python 3.10〜3.12（システム、推奨3.12） | installer.py 自体は GUI 同梱の Python で動くため**不要になる**。ただし ESP-IDF 自身のインストーラが必要とする場合があるため、無ければ警告+導入案内（エラーにはしない）。無い場合は winget（Windows）/ Homebrew（macOS）経由のワンクリック自動インストールボタンも提示する（Linuxはコマンド案内のみ） |
