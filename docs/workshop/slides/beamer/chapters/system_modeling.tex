...
\begin{frame}{$\tau_m$ と $K_m$ の導出 / Deriving $\tau_m$ and $K_m$}
  ...
  \begin{columns}[t]
    \begin{column}{0.48\textwidth}
      ...
      \end{block}
      \vspace{-0.2em}
      % 2026-07-27 実測ファミリへ更新(stampfly_physical.yaml)
      % Dm はコーストダウン単体では検出限界以下(≈0)だが、ホバー点では
      % プロペラ空力減衰 2*Cq*omega_h(omega_h≈3670rad/s)が実効的に効くため、
      % 表の Dm はこの空力優勢の実効値(2Cq*omega_h)を示す
      {\scriptsize
      \begin{tabular}{lll}
        \hline
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{tabular}{lll}
        ...
        $L$         & 0.023\,m & アーム長 \\
        $\kappa$    & 6.12e-3\,m & トルク推力比 \\
        \hline
        $K_{m,rp}$  & \textbf{9.3e-4}\,N$\cdot$m & Roll/Pitch \\
        $K_{m,yaw}$ & \textbf{2.4e-4}\,N$\cdot$m & Yaw \\
        \hline
      \end{tabular}}
    \end{column}
  \end{columns}
  ...
  \centering
  実効プラントゲイン: \; $K = K_m / I$
  \quad ($K_{roll}\!=\!102$,\; $K_{pitch}\!=\!70$,\; $K_{yaw}\!=\!12$\,rad/s$^2$)
\end{frame}
...
\begin{frame}{パラメータ一覧 / Parameter Summary}
  \begin{table}
    ...
    \begin{tabular}{llccc}
      ...
      % 2026-07-27 実測ファミリへ更新(stampfly_physical.yaml)
      トルクゲイン & $K_m$ & 9.3e-4 & 9.3e-4 & 2.4e-4\,N$\cdot$m \\
      モータ時定数 & $\tau_m$ & \multicolumn{3}{c}{0.02\,s（共通、実測 $\tau_{eff}\!\approx\!0.0163$\,s）} \\
      \hline
      \textbf{プラントゲイン} & $\bm{K = K_m/I}$
        & \textbf{102} & \textbf{70} & \textbf{12}\,rad/s$^2$ \\
      \hline
    \end{tabular}
  \end{table}
...
\begin{frame}{ゲイン設計表 / Gain Design Table}
  \begin{table}
    ...
    \begin{tabular}{lcccc}
      ...
      % 2026-07-27 実測ファミリへ更新(stampfly_physical.yaml): kappa変更でK_yaw 19→12
      Yaw   &  12 & 1.44 & 2.13 & 1.04 \\
      \hline
    \end{tabular}
  \end{table}
...
\begin{frame}[fragile]{実習: モデルベースゲイン設計 / Hands-on}
  \begin{sflisting}{user\_code.cpp (excerpt)}
// Model-based Kp design (target zeta = 0.7)
// 2026-07-27 実測ファミリへ更新(stampfly_physical.yaml): K_yaw 19.0f→12.0f
const float tau_m = 0.02f;
const float K_roll  = 102.0f;
const float K_pitch =  70.0f;
const float K_yaw   =  12.0f;
float zeta = 0.7f;
