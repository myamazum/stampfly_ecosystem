...
\begin{frame}{② 書き込みの仕組み --- 3つの方法}
  ...
  {\footnotesize 今日の実習は②・③とも sf CLI で書き込みます。Web / GUI フラッシャは\textbf{開発環境のない PC でも使える}方法（紹介のみ）}
\end{frame}
...
\begin{frame}{② StampFly の入り口 --- Landing ページ}
  \begin{columns}[c]
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\footnotesize 学校の PC に開発環境がなくても、このページから書き込みは試せます（今日は紹介のみ）}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{② よくある失敗と対処}
  ...
  \begin{exampleblock}{安心してください}
    \footnotesize \code{sf flash} は機体の設定（ペアリング等）を消さない書き込みなので、何度失敗してもやり直せます
  \end{exampleblock}
\end{frame}
