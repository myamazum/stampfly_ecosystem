...
\begin{tikzpicture}[
  ...
  \begin{scope}[rotate around={143:(-2.85,2.15)}]
    \draw[mom] ([shift={(-2.85,2.15)}] -40:0.38 and 0.5)
      arc[x radius=0.38, y radius=0.5, start angle=-40, end angle=185];
  \end{scope}
  \node[lbl, text=cmom] at (-3.45,2.55) {ロール};

  % Pitch (around lateral axis) / ピッチ（左右軸まわり）
  \begin{scope}[rotate around={11:(2.95,0.60)}]
    \draw[mom] ([shift={(2.95,0.60)}] -40:0.38 and 0.5)
      arc[x radius=0.38, y radius=0.5, start angle=-40, end angle=185];
  \end{scope}
  \node[lbl, text=cmom] at (3.55,1.15) {ピッチ};

  % Yaw (around vertical axis) / ヨー（上下軸まわり）
  \draw[mom] ([shift={(0,-1.75)}] -30:0.6 and 0.24)
    arc[x radius=0.6, y radius=0.24, start angle=-30, end angle=200];
