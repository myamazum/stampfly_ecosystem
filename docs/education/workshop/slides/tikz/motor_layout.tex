...
\begin{tikzpicture}[
  ...
  % M1 - Front Right (CW)
  \node[cw] (m1) at (2.5, 2.5) {M1};
  \node[label, above right=-1mm] at (m1.north east) {FR};
  \draw[arrow, cwcolor] ([shift={(45:1.3)}]m1.center)
    arc[start angle=45, end angle=315, radius=1.3cm];
  \node[font=\scriptsize, cwcolor] at (4.2, 2.5) {CW};

  % M2 - Rear Right (CCW)
  \node[ccw] (m2) at (2.5, -2.5) {M2};
  \node[label, below right=-1mm] at (m2.south east) {RR};
  \draw[arrow, ccwcolor] ([shift={(-45:1.3)}]m2.center)
    arc[start angle=-45, end angle=-315, radius=1.3cm];
  \node[font=\scriptsize, ccwcolor] at (4.2, -2.5) {CCW};

  % M3 - Rear Left (CW)
  \node[cw] (m3) at (-2.5, -2.5) {M3};
  \node[label, below left=-1mm] at (m3.south west) {RL};
  \draw[arrow, cwcolor] ([shift={(225:1.3)}]m3.center)
    arc[start angle=225, end angle=-45, radius=1.3cm];
  \node[font=\scriptsize, cwcolor] at (-4.2, -2.5) {CW};

  % M4 - Front Left (CCW)
  \node[ccw] (m4) at (-2.5, 2.5) {M4};
  \node[label, above left=-1mm] at (m4.north west) {FL};
  \draw[arrow, ccwcolor] ([shift={(135:1.3)}]m4.center)
