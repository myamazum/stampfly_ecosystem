...
## 2〜5章用の追加検証（2026-08-02）

| 概念 | 採用表記 | 備考 |
|------|---------|------|
| 姿勢推定の誤差状態フィルタ | 初出「誤差状態カルマンフィルタ（Error-State Kalman Filter, ESKF）」→以後 ESKF | 和訳語の査読実例未発見（東大研究室の技術資料訳のみ）。英字定義で導入 |
| 相補フィルタ | 相補フィルタ | 慣用確認（SICE論文集53-5ほか） |
| 外乱推定に基づく補償 | 外乱オブザーバ | 慣用確認（JRSJ 11-4 大西ほか、多数） |
| 積分飽和対策 | アンチワインドアップ | 慣用確認（電学論D 125-1ほか多数）。現象は「ワインドアップ」 |
| 微分項のLPF | 不完全微分 | 慣用確認（PID教科書・JACC52） |
| モータへの推力配分 | **定訳なし**→初出で「各モータへの推力配分（以下ミキサ）」と説明的導入 | 「制御配分/コントロールアロケーション」のマルチロータ査読実例未発見。ヘリコプタ工学の「ミキサ」（JSASS技術誌）が最近接 |
| 内外ループの応答速度差 | **「時間スケール分離」は実例未発見**→「内側ループの応答を外側ループより十分高速にする」等の説明的表現 | 固定術語として使わない |
| 発振の閾値 | **「安定境界」は実例未発見**→「安定限界」（閾値）/「安定余裕」（マージン） | JJSASS 51-597「安定余裕」実例あり |
| 持続振動 | リミットサイクル | 慣用確認（SICE誌 1962〜） |
| モータ種別 | コアレスモータ | 慣用確認（電学論122-8ほか） |
| PWMのオン比率 | デューティ比 | 慣用確認（ISCIE 28-7ほか） |
| 保持の正確さ | 「位置保持の精度」等、確立語＋説明句で組み立て | 「ホバリング精度」「位置保持精度」の複合語は査読実例未発見 |
| 回転の4元数表現 | クォータニオン | JACC51「クォータニオンを用いた小型電動ヘリコプタの姿勢制御」が最近接先例。数理系では「四元数」も慣用 |
| 同定結果をモデルへ | **「還流」禁止（実例なし）**→「反映する」「モデルを更新する」 | 開発文書の内部用語。論文では使わない |

（検証方法の注意: WebSearch経由のため「未発見」=不存在の証明ではない。原本=セッションscratchpad terminology_ch2_5.md）
