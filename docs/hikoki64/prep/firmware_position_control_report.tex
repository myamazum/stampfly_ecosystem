...
速度ループが見る合計遅れ$\approx110$msのうち、アクチュエーション側$\approx82$ms（$L$=62ms+$T$=20ms）に対し、ESKF速度推定側が$\approx28$ms加わる（\texttt{README.md:85-86}、第4.6節）。DOBはIMU比力を直接使用することでこのESKF側の遅れを回避する設計である。実機A/Bで高度std $-67\%$（55.2 vs 167.2mm、第7章）を確認したが、既定はオフ（オプトイン）である（第3.6節）。
...
2026-06-27 \newline\texttt{poshold\_3min\_battery\_\allowbreak wobble\_20260627.md}（178秒の3分飛行） & 水平位置保持 & 平均drift約13cm, std 4.2cm, 最大24.5cm, 卓越周波数0.09Hz \\
 ...
 & 電池電圧推移 & 地上4.168V→離陸直後3.872V→最終3.488V（約$-2.1$mV/秒の単調サグ） \\
\midrule
