/**
 * @file eskf_v2_test.cpp
 * @brief ESKF_V2 Unit Tests
 *
 * Test categories:
...
// ============================================================================
static ESKF_V2::Config allSensorsOnConfig() {
    auto cfg = ESKF_V2::Config::defaultConfig();
    cfg.sensor_enabled[ESKF_V2::SENSOR_MAG]  = true;
    cfg.sensor_enabled[ESKF_V2::SENSOR_BARO] = true;
    cfg.sensor_enabled[ESKF_V2::SENSOR_TOF]  = true;
    cfg.sensor_enabled[ESKF_V2::SENSOR_FLOW] = true;
...
void test_mag_off_isolation() {
    ...
    ESKF_V2 eskf;
    auto config = allSensorsOnConfig();
    config.sensor_enabled[ESKF_V2::SENSOR_MAG] = false;
    eskf.init(config);

    // Verify active mask
    uint16_t mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::ATT_Z) & 1) == 0, "ATT_Z frozen when Mag OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::BG_Z) & 1) == 0, "BG_Z frozen when Mag OFF");
    // Other states should be active
    ASSERT_TRUE(((mask >> ESKF_V2::ATT_X) & 1) == 1, "ATT_X active when Mag OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::BG_X) & 1) == 1, "BG_X active when Mag OFF");
    ...
    for (int j = 0; j < 15; j++) {
        if (j == ESKF_V2::ATT_Z) continue;
        if (std::fabs(P(ESKF_V2::ATT_Z, j)) > 1e-10f) {
    ...
    for (int j = 0; j < 15; j++) {
        if (j == ESKF_V2::BG_Z) continue;
        if (std::fabs(P(ESKF_V2::BG_Z, j)) > 1e-10f) {
    ...
    ASSERT_NEAR(att_var, P(ESKF_V2::ATT_Z, ESKF_V2::ATT_Z), 1e-6f, "ATT_Z diagonal = init value");
    ASSERT_NEAR(bg_var, P(ESKF_V2::BG_Z, ESKF_V2::BG_Z), 1e-6f, "BG_Z diagonal = init value");

    // Active states should have non-initial covariance (predict changed them)
    ASSERT_TRUE(P(ESKF_V2::ATT_X, ESKF_V2::ATT_X) != att_var,
...
void test_flow_off_isolation() {
    ...
    ESKF_V2 eskf;
    auto config = allSensorsOnConfig();
    config.sensor_enabled[ESKF_V2::SENSOR_FLOW] = false;
    eskf.init(config);

    uint16_t mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::POS_X) & 1) == 0, "POS_X frozen when Flow OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::POS_Y) & 1) == 0, "POS_Y frozen when Flow OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::VEL_X) & 1) == 0, "VEL_X frozen when Flow OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::VEL_Y) & 1) == 0, "VEL_Y frozen when Flow OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::BA_X) & 1) == 0, "BA_X frozen when Flow OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::BA_Y) & 1) == 0, "BA_Y frozen when Flow OFF");
    // Z states should remain active (Baro+ToF ON)
    ASSERT_TRUE(((mask >> ESKF_V2::POS_Z) & 1) == 1, "POS_Z active (Baro/ToF ON)");
    ASSERT_TRUE(((mask >> ESKF_V2::VEL_Z) & 1) == 1, "VEL_Z active (Baro/ToF ON)");
    ...
    int frozen_states[] = {ESKF_V2::POS_X, ESKF_V2::POS_Y,
                           ESKF_V2::VEL_X, ESKF_V2::VEL_Y,
                           ESKF_V2::BA_X, ESKF_V2::BA_Y};
    ...
    ASSERT_NEAR(pos_var, P(ESKF_V2::POS_X, ESKF_V2::POS_X), 1e-6f, "POS_X diag = init");
    ASSERT_NEAR(vel_var, P(ESKF_V2::VEL_X, ESKF_V2::VEL_X), 1e-6f, "VEL_X diag = init");
    ASSERT_NEAR(ba_var, P(ESKF_V2::BA_X, ESKF_V2::BA_X), 1e-6f, "BA_X diag = init");
...
void test_baro_tof_off_isolation() {
    ...
    ESKF_V2 eskf;
    auto config = allSensorsOnConfig();
    config.sensor_enabled[ESKF_V2::SENSOR_BARO] = false;
    config.sensor_enabled[ESKF_V2::SENSOR_TOF]  = false;
    eskf.init(config);

    uint16_t mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::POS_Z) & 1) == 0, "POS_Z frozen when Baro+ToF OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::VEL_Z) & 1) == 0, "VEL_Z frozen when Baro+ToF OFF");
    ASSERT_TRUE(((mask >> ESKF_V2::BA_Z)  & 1) == 0, "BA_Z frozen when Baro+ToF OFF");

    // If only Baro OFF but ToF ON → Z states should be active
    eskf.setSensorEnabled(ESKF_V2::SENSOR_TOF, true);
    mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::POS_Z) & 1) == 1, "POS_Z active when ToF ON (Baro OFF)");
    ASSERT_TRUE(((mask >> ESKF_V2::VEL_Z) & 1) == 1, "VEL_Z active when ToF ON (Baro OFF)");
...
void test_dynamic_sensor_toggle() {
    ...
    float cross_before = std::fabs(P(ESKF_V2::ATT_Z, ESKF_V2::ATT_X));
    printf("  Before Mag OFF: P(ATT_Z,ATT_X) = %.2e\n", cross_before);

    // Turn Mag OFF → ATT_Z, BG_Z should freeze
    eskf.setSensorEnabled(ESKF_V2::SENSOR_MAG, false);

    // Cross-covariance should now be zero
    ASSERT_NEAR(0.0f, P(ESKF_V2::ATT_Z, ESKF_V2::ATT_X), 1e-10f,
                "ATT_Z cross-cov zeroed after Mag OFF");
    ASSERT_NEAR(att_var_init, P(ESKF_V2::ATT_Z, ESKF_V2::ATT_Z), 1e-6f,
    ...
    eskf.setSensorEnabled(ESKF_V2::SENSOR_MAG, true);
    uint16_t mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::ATT_Z) & 1) == 1, "ATT_Z active after Mag ON");
    ASSERT_TRUE(((mask >> ESKF_V2::BG_Z) & 1) == 1, "BG_Z active after Mag ON");
    ...
    float cross_att_bg = std::fabs(P(ESKF_V2::ATT_Z, ESKF_V2::BG_Z));
...
void test_freeze_accel_bias() {
    ...
    uint16_t mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::BA_X) & 1) == 0, "BA_X frozen at startup (freeze=true)");
    ...
    mask = eskf.getActiveMask();
    ASSERT_TRUE(((mask >> ESKF_V2::BA_X) & 1) == 0, "BA_X frozen after resetForLanding");
    ASSERT_TRUE(((mask >> ESKF_V2::BA_Y) & 1) == 0, "BA_Y frozen after resetForLanding");
    ASSERT_TRUE(((mask >> ESKF_V2::BA_Z) & 1) == 0, "BA_Z frozen after resetForLanding");
...
void test_q_gating() {
    ...
    ASSERT_NEAR(bg_var_init, P(ESKF_V2::BG_Z, ESKF_V2::BG_Z), 1e-6f,
                "BG_Z P unchanged after 4000 predict (Q gated)");

    // ATT_X should have grown (active, Q added)
    ASSERT_TRUE(P(ESKF_V2::ATT_X, ESKF_V2::ATT_X) > att_var_init,
