...
>
> **【追記 2026-07-15】パラメータ再測定による数値更新**: コーストダウン試験・プロペラ写真の画素直接積分・回転子実測諸元により $J_{mp}$=**1.375e-8**（旧2.01e-8）、$C_q$=**4.10e-11**（旧9.71e-11）、$\omega_0$=**3670 rad/s**（旧2930）と確定（multicopter_introduction/notes/qa_log.md Q4-9..13）。再計算: **$\tau_z = J_{mp}/(2C_q\omega_0) = 45.7$ ms (3.5 Hz)**。$K_e$=**5.33e-4** で確定（コーストダウン開回路EMFの直接測定。独立の無負荷V-ω同定 5.35e-4 と0.5%一致）。$R$=0.593（論文LCR）。γ=1 で $T_m$=17.6 ms、飛行同定の 20〜34 ms は実効導通率 γ≈0.2〜0.8 に相当。交差 2.9 Hz のリード予測は +22.0° となり、フライト実測 +22〜32° の帯域内で**整合**。結論（LHP・設計は積分器+遅れ）は不変。
>
> ### 同定の限界（実データで証明）
