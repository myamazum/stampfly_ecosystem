...
inline constexpr float ACCEL_NOISE = 0.5f;              // 加速度ノイズ [m/s²/√Hz] (自動調整: NLL最小+白色イノベーション)
inline constexpr float GYRO_BIAS_NOISE = 0.000013f;    // ジャイロバイアスランダムウォーク
inline constexpr float ACCEL_BIAS_NOISE = 0.01f;        // 加速度バイアスランダムウォーク [m/s²/√s] (自動調整: BA追従+安定性)
