...
inline constexpr float ACCEL_NOISE = 0.062885f;         // 加速度ノイズ [m/s²/√Hz] (IMU+ToF時代の安定値)
inline constexpr float GYRO_BIAS_NOISE = 0.000013f;    // ジャイロバイアスランダムウォーク
inline constexpr float ACCEL_BIAS_NOISE = 0.001f;       // 加速度バイアスランダムウォーク [m/s²/√s] (IMU+ToF時代の値)
...
inline constexpr float FLOW_NOISE = 0.01f;             // オプティカルフローノイズ [m/s] (IMU+ToF時代の値)
inline constexpr float ACCEL_ATT_NOISE = 0.06f;        // 加速度計姿勢補正ノイズ [m/s²] (IMU+ToF時代の安定値)
