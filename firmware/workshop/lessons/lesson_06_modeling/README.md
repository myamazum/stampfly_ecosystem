...
> （J_prop = 1.03e-8）、回転子実測諸元（J_rotor = 3.45e-9）、Ke = 5.5e-4（無負荷V-ω実測、
> コーストダウン開回路EMFでも計器較正補正後に一致）から、ホバ点の実効時定数は
> 完全導通（γ=1）で ≈0.017 s、飛行同定の 0.02〜0.034 s は γ≈0.2〜0.76 に相当し、
> 本表の 0.02 は実測と整合する。
