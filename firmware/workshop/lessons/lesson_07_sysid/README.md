...
1. `user_code.cpp` を開く
2. $K_p$ を設定（例: 0.5、L5 で使った値）
3. WiFi チャンネルを設定
4. ビルド & 書き込み: `sf build workshop` → `sf flash vehicle`
...
1. Open `user_code.cpp`
2. Set $K_p$ (e.g., 0.5, same as L5)
3. Set WiFi channel
4. Build & flash: `sf build workshop` → `sf flash vehicle`
